  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach(doc => {
        ...
        if (data.teacher?.email && data.teacher.email.toLowerCase().trim() === email) {
        ...
        if (data.levels && Array.isArray(data.levels)) {
          data.levels.forEach((lvl: { name: string, teacher?: TeacherData }) => {
            if (lvl.teacher?.email && lvl.teacher.email.toLowerCase().trim() === email) {
              taughtAcademies.push({
                academyId: doc.id,
                academyName: academyName,
                level: lvl.name,
...
function userDisplayName(user: { displayName?: string | null, email?: string | null }) {
